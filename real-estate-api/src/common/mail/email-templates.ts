...
export function appointmentCreatedHtml(
  name: string,
  date: string,
  property?: string,
): string {
  return wrap(`
    ...
    ${table(
      (property ? row('🏠 Bất động sản', property) : '') +
        ...
        row(
          '📋 Trạng thái',
          `<span style="color:${WARN};font-weight:700;">Chờ xác nhận</span>`,
        ),
    )}
...
export function appointmentApprovedHtml(
  name: string,
  date: string,
  property?: string,
): string {
  return wrap(`
    ...
    ${table(
      (property ? row('🏠 Bất động sản', property) : '') +
        ...
        row(
          '📋 Trạng thái',
          `<span style="color:${OK};font-weight:700;">Đã duyệt</span>`,
        ),
    )}
...
export function appointmentRejectedHtml(
  name: string,
  date: string,
  property?: string,
  reason?: string,
  ...
  return wrap(`
    ...
    ${table(
      row('📰 Bài đăng', title) +
        row(
          '📋 Trạng thái',
          `<span style="color:${OK};font-weight:700;">Đã duyệt — Đang hiển thị</span>`,
        ),
    )}
  ...
  return wrap(`
    ...
    ${table(
      row('📰 Bài đăng', title) +
        row(
          '📋 Trạng thái',
          `<span style="color:${ERR};font-weight:700;">Chưa duyệt</span>`,
        ),
    )}
  ...
  return wrap(`
    ...
    ${table(
      row('📦 Gói dịch vụ', pkg) +
        ...
        row(
          '💰 Số tiền',
          `<span style="color:${OK};font-size:18px;font-weight:700;">${fmtMoney(amount)}</span>`,
        ) +
...
export function paymentFailureHtml(
  name: string,
  amount: number,
  pkg: string,
  postTitle?: string,
  ...
  return wrap(`
    ...
    ${table(
      row('📦 Gói dịch vụ', pkg) +
        ...
        row(
          '💰 Số tiền',
          `<span style="color:${ERR};font-size:18px;font-weight:700;">${fmtMoney(amount)}</span>`,
        ) +
        (method ? row('💳 Phương thức', method.toUpperCase()) : ''),
    )}
...
export function depositSuccessHtml(
  name: string,
  property: string,
  amount: number,
  expiresAt: string,
  ...
  return wrap(`
    ...
    ${
      isAfterViewing
        ? note(
            '⚠️',
            'Đây là cọc chốt mua sau khi xem. Số tiền này không được hoàn trả.',
            '#fef3c7',
            WARN,
            '#92400e',
          )
        : note(
            'ℹ️',
            'Bạn có thể yêu cầu hoàn tiền trước khi hết hạn. Tỷ lệ hoàn tùy thuộc thời điểm hủy.',
            '#eff6ff',
            '#3b82f6',
            '#1e40af',
          )
...
export function refundApprovedHtml(
  name: string,
  property: string,
  amount: number,
): string {
...
export function refundRejectedHtml(
  name: string,
  property: string,
  reason?: string,
): string {
  return wrap(`
    ...
    ${table(
      row('🏠 Bất động sản', property) +
        ...
        row(
          '📋 Trạng thái',
          `<span style="color:${ERR};font-weight:700;">Từ chối — Vẫn giữ chỗ</span>`,
        ),
    )}
