...
const CON_GIAP: Record<string, { ten: string; emoji: string; tinhCach: string }> = {
    'Tý':   { ten: 'Chuột', emoji: '🐭', tinhCach: 'Thông minh, nhanh nhẹn, khéo léo và có khiếu kinh doanh.' },
    'Sửu':  { ten: 'Trâu',  emoji: '🐂', tinhCach: 'Chăm chỉ, kiên nhẫn, đáng tin cậy và có trách nhiệm.' },
    'Dần':  { ten: 'Hổ',    emoji: '🐯', tinhCach: 'Dũng cảm, tự tin, nhiệt tình và có tinh thần lãnh đạo.' },
    'Mão':  { ten: 'Mèo',   emoji: '🐰', tinhCach: 'Tinh tế, nhẹ nhàng, khéo léo và có óc thẩm mỹ cao.' },
    'Thìn': { ten: 'Rồng',  emoji: '🐲', tinhCach: 'Tự tự, tham vọng, may mắn và có sức hút tự nhiên.' },
    'Tỵ':   { ten: 'Rắn',   emoji: '🐍', tinhCach: 'Khôn ngoan, trực giác tốt, bí ẩn và sâu sắc.' },
    'Ngọ':  { ten: 'Ngựa',  emoji: '🐴', tinhCach: 'Năng động, tự do, nhiệt huyết và thích phiêu lưu.' },
    'Mùi':  { ten: 'Dê',    emoji: '🐑', tinhCach: 'Hiền lành, sáng tạo, nhạy cảm và yêu nghệ thuật.' },
    'Thân': { ten: 'Khỉ',   emoji: '🐒', tinhCach: 'Thông minh, hài hước, linh hoạt và tháo vát.' },
    'Dậu':  { ten: 'Gà',    emoji: '🐓', tinhCach: 'Cần cù, chính xác, tự tin và có óc quan sát.' },
    'Tuất': { ten: 'Chó',   emoji: '🐕', tinhCach: 'Trung thành, chân thật, dũng cảm và đáng tin cậy.' },
    'Hợi':  { ten: 'Lợn',   emoji: '🐷', tinhCach: 'Hào phóng, chân thành, vui vẻ và có lòng tốt.' },
};

// --- DATA: BÁT TRẠCH CHI TIẾT ---
const SAO_BAT_TRACH = {
    SinhKhi:   { ten: 'Sinh Khí',   loai: 'Cát', moTa: 'Thu hút tài lộc, danh tiếng và thăng quan phát tài. Phù hợp nhất làm hướng cửa chính hoặc phòng làm việc để kích hoạt năng lượng tích cực nhất.' },
    ThienY:    { ten: 'Thiên Y',    loai: 'Cát', moTa: 'Cải thiện sức khỏe, trường thọ. Mang lại tinh thần minh mẫn, gặp may mắn có quý nhân phù trợ. Đặc biệt tốt cho không gian phòng ngủ của gia chủ.' },
    DienNien:  { ten: 'Diên Niên',  loai: 'Cát', moTa: 'Củng cố các mối quan hệ trong gia đình và tình yêu. Xây dựng sự hòa thuận, gắn kết giữa các thành viên. Lý tưởng cho không gian sinh hoạt chung.' },
    PhucVi:    { ten: 'Phục Vị',    loai: 'Cát', moTa: 'Củng cố sức mạnh tinh thần, mang lại tiến bộ của bản thân và may mắn trong thi cử. Hướng này mang lại sự bình yên, phù hợp làm nơi thờ tự hoặc đọc sách.' },
    TuyetMenh: { ten: 'Tuyệt Mệnh', loai: 'Hung', moTa: 'Hướng xấu nhất, có thể gây phá sản, bệnh tật chết người. Tuyệt đối không đặt cửa chính hoặc đầu giường theo hướng này.' },
    NguQuy:    { ten: 'Ngũ Quỷ',    loai: 'Hung', moTa: 'Mất nguồn thu nhập, mất việc làm, cãi lộn. Dễ gặp phải những rủi ro về hỏa hoạn hoặc trộm cắp, tốn hao năng lượng và tiền bạc.' },
    LucSat:    { ten: 'Lục Sát',    loai: 'Hung', moTa: 'Xáo trộn trong quan hệ tình cảm, thù hận, kiện tụng và tai nạn. Gây ra sự bất an về tinh thần và những rắc rối thị phi không đáng có.' },
    HoaHai:    { ten: 'Họa Hại',    loai: 'Hung', moTa: 'Không may mắn, thị phi, thất bại. Gây ra những trở ngại nhỏ nhưng liên tục, làm tiêu hao nghị lực và sự kiên trì trong công việc.' }
};

const BATTRACH_MAP: Record<number, Record<string, { ten: string; loai: string; moTa: string; huong: string }>> = {
    1: { 'Đông Nam': {...SAO_BAT_TRACH.SinhKhi, huong: 'Đông Nam'}, 'Đông': {...SAO_BAT_TRACH.ThienY, huong: 'Đông'}, 'Nam': {...SAO_BAT_TRACH.DienNien, huong: 'Nam'}, 'Bắc': {...SAO_BAT_TRACH.PhucVi, huong: 'Bắc'}, 'Tây Nam': {...SAO_BAT_TRACH.TuyetMenh, huong: 'Tây Nam'}, 'Đông Bắc': {...SAO_BAT_TRACH.NguQuy, huong: 'Đông Bắc'}, 'Tây Bắc': {...SAO_BAT_TRACH.LucSat, huong: 'Tây Bắc'}, 'Tây': {...SAO_BAT_TRACH.HoaHai, huong: 'Tây'} }, // Khảm
    2: { 'Đông Bắc': {...SAO_BAT_TRACH.SinhKhi, huong: 'Đông Bắc'}, 'Tây': {...SAO_BAT_TRACH.ThienY, huong: 'Tây'}, 'Tây Bắc': {...SAO_BAT_TRACH.DienNien, huong: 'Tây Bắc'}, 'Tây Nam': {...SAO_BAT_TRACH.PhucVi, huong: 'Tây Nam'}, 'Bắc': {...SAO_BAT_TRACH.TuyetMenh, huong: 'Bắc'}, 'Đông Nam': {...SAO_BAT_TRACH.NguQuy, huong: 'Đông Nam'}, 'Nam': {...SAO_BAT_TRACH.LucSat, huong: 'Nam'}, 'Đông': {...SAO_BAT_TRACH.HoaHai, huong: 'Đông'} }, // Khôn
    3: { 'Nam': {...SAO_BAT_TRACH.SinhKhi, huong: 'Nam'}, 'Bắc': {...SAO_BAT_TRACH.ThienY, huong: 'Bắc'}, 'Đông Nam': {...SAO_BAT_TRACH.DienNien, huong: 'Đông Nam'}, 'Đông': {...SAO_BAT_TRACH.PhucVi, huong: 'Đông'}, 'Tây': {...SAO_BAT_TRACH.TuyetMenh, huong: 'Tây'}, 'Tây Bắc': {...SAO_BAT_TRACH.NguQuy, huong: 'Tây Bắc'}, 'Đông Bắc': {...SAO_BAT_TRACH.LucSat, huong: 'Đông Bắc'}, 'Tây Nam': {...SAO_BAT_TRACH.HoaHai, huong: 'Tây Nam'} }, // Chấn
    4: { 'Bắc': {...SAO_BAT_TRACH.SinhKhi, huong: 'Bắc'}, 'Nam': {...SAO_BAT_TRACH.ThienY, huong: 'Nam'}, 'Đông': {...SAO_BAT_TRACH.DienNien, huong: 'Đông'}, 'Đông Nam': {...SAO_BAT_TRACH.PhucVi, huong: 'Đông Nam'}, 'Đông Bắc': {...SAO_BAT_TRACH.TuyetMenh, huong: 'Đông Bắc'}, 'Tây Nam': {...SAO_BAT_TRACH.NguQuy, huong: 'Tây Nam'}, 'Tây': {...SAO_BAT_TRACH.LucSat, huong: 'Tây'}, 'Tây Bắc': {...SAO_BAT_TRACH.HoaHai, huong: 'Tây Bắc'} }, // Tốn
    6: { 'Tây': {...SAO_BAT_TRACH.SinhKhi, huong: 'Tây'}, 'Đông Bắc': {...SAO_BAT_TRACH.ThienY, huong: 'Đông Bắc'}, 'Tây Nam': {...SAO_BAT_TRACH.DienNien, huong: 'Tây Nam'}, 'Tây Bắc': {...SAO_BAT_TRACH.PhucVi, huong: 'Tây Bắc'}, 'Nam': {...SAO_BAT_TRACH.TuyetMenh, huong: 'Nam'}, 'Đông': {...SAO_BAT_TRACH.NguQuy, huong: 'Đông'}, 'Bắc': {...SAO_BAT_TRACH.LucSat, huong: 'Bắc'}, 'Đông Nam': {...SAO_BAT_TRACH.HoaHai, huong: 'Đông Nam'} }, // Càn
    7: { 'Tây Bắc': {...SAO_BAT_TRACH.SinhKhi, huong: 'Tây Bắc'}, 'Tây Nam': {...SAO_BAT_TRACH.ThienY, huong: 'Tây Nam'}, 'Đông Bắc': {...SAO_BAT_TRACH.DienNien, huong: 'Đông Bắc'}, 'Tây': {...SAO_BAT_TRACH.PhucVi, huong: 'Tây'}, 'Đông': {...SAO_BAT_TRACH.TuyetMenh, huong: 'Đông'}, 'Nam': {...SAO_BAT_TRACH.NguQuy, huong: 'Nam'}, 'Đông Nam': {...SAO_BAT_TRACH.LucSat, huong: 'Đông Nam'}, 'Bắc': {...SAO_BAT_TRACH.HoaHai, huong: 'Bắc'} }, // Đoài
    8: { 'Tây Nam': {...SAO_BAT_TRACH.SinhKhi, huong: 'Tây Nam'}, 'Tây Bắc': {...SAO_BAT_TRACH.ThienY, huong: 'Tây Bắc'}, 'Tây': {...SAO_BAT_TRACH.DienNien, huong: 'Tây'}, 'Đông Bắc': {...SAO_BAT_TRACH.PhucVi, huong: 'Đông Bắc'}, 'Đông Nam': {...SAO_BAT_TRACH.TuyetMenh, huong: 'Đông Nam'}, 'Bắc': {...SAO_BAT_TRACH.NguQuy, huong: 'Bắc'}, 'Đông': {...SAO_BAT_TRACH.LucSat, huong: 'Đông'}, 'Nam': {...SAO_BAT_TRACH.HoaHai, huong: 'Nam'} }, // Cấn
    9: { 'Đông': {...SAO_BAT_TRACH.SinhKhi, huong: 'Đông'}, 'Đông Nam': {...SAO_BAT_TRACH.ThienY, huong: 'Đông Nam'}, 'Bắc': {...SAO_BAT_TRACH.DienNien, huong: 'Bắc'}, 'Nam': {...SAO_BAT_TRACH.PhucVi, huong: 'Nam'}, 'Tây Bắc': {...SAO_BAT_TRACH.TuyetMenh, huong: 'Tây Bắc'}, 'Tây': {...SAO_BAT_TRACH.NguQuy, huong: 'Tây'}, 'Tây Nam': {...SAO_BAT_TRACH.LucSat, huong: 'Tây Nam'}, 'Đông Bắc': {...SAO_BAT_TRACH.HoaHai, huong: 'Đông Bắc'} }, // Ly
};

// --- BẢNG CUNG SỐ ---
const CUNG_SO_INFO: Record<number, { ten: string; menh: string; huongTot: string[]; huongXau: string[]; mauSac: string[]; vatLieu: string[]; moTa: string; soMayMan: string }> = {
    1: { ten: 'Cung Khảm', menh: 'Thủy', huongTot: ['Đông Nam', 'Đông', 'Nam', 'Bắc'], huongXau: ['Tây Nam', 'Đông Bắc', 'Tây Bắc', 'Tây'], mauSac: ['Xanh lam', 'Đen', 'Trắng'], vatLieu: ['Kính', 'Kim loại', 'Thủy tinh'], moTa: 'Cung Khảm thuộc Thủy, chủ về trí tuệ và sự nghiệp.', soMayMan: '1, 4, 6, 7' },
    2: { ten: 'Cung Khôn', menh: 'Thổ', huongTot: ['Đông Bắc', 'Tây', 'Tây Bắc', 'Tây Nam'], huongXau: ['Bắc', 'Đông Nam', 'Nam', 'Đông'], mauSac: ['Vàng', 'Nâu', 'Đỏ'], vatLieu: ['Đất nung', 'Gạch', 'Gốm sứ'], moTa: 'Cung Khôn thuộc Thổ, chủ về sức khỏe và gia đình.', soMayMan: '2, 5, 8, 9' },
    3: { ten: 'Cung Chấn', menh: 'Mộc', huongTot: ['Nam', 'Bắc', 'Đông Nam', 'Đông'], huongXau: ['Tây', 'Tây Bắc', 'Đông Bắc', 'Tây Nam'], mauSac: ['Xanh lá', 'Xanh lam', 'Đen'], vatLieu: ['Gỗ tự nhiên', 'Tre', 'Mây'], moTa: 'Cung Chấn thuộc Mộc, chủ về phát triển và sinh lực.', soMayMan: '1, 3, 4' },
    4: { ten: 'Cung Tốn', menh: 'Mộc', huongTot: ['Bắc', 'Nam', 'Đông', 'Đông Nam'], huongXau: ['Đông Bắc', 'Tây Nam', 'Tây', 'Tây Bắc'], mauSac: ['Xanh lá', 'Xanh lam', 'Đen'], vatLieu: ['Gỗ', 'Vải lụa', 'Tre'], moTa: 'Cung Tốn thuộc Mộc, chủ về tài lộc và may mắn.', soMayMan: '1, 3, 4' },
    5: { ten: 'Cung Trung Cung', menh: 'Thổ', huongTot: [], huongXau: [], mauSac: ['Vàng', 'Nâu', 'Be'], vatLieu: ['Đá', 'Gạch', 'Đất sét'], moTa: 'Cung Trung Cung thuộc Thổ, là trung tâm của bát quái.', soMayMan: '2, 5, 8' },
    6: { ten: 'Cung Càn', menh: 'Kim', huongTot: ['Tây', 'Đông Bắc', 'Tây Nam', 'Tây Bắc'], huongXau: ['Nam', 'Đông', 'Bắc', 'Đông Nam'], mauSac: ['Trắng', 'Vàng kim', 'Bạc'], vatLieu: ['Kim loại', 'Inox', 'Nhôm'], moTa: 'Cung Càn thuộc Kim, chủ về quyền lực và lãnh đạo.', soMayMan: '2, 5, 6, 7, 8' },
    7: { ten: 'Cung Đoài', menh: 'Kim', huongTot: ['Tây Bắc', 'Tây Nam', 'Đông Bắc', 'Tây'], huongXau: ['Đông', 'Nam', 'Đông Nam', 'Bắc'], mauSac: ['Trắng', 'Bạc', 'Vàng nhạt'], vatLieu: ['Kim loại', 'Kính', 'Gốm sứ trắng'], moTa: 'Cung Đoài thuộc Kim, chủ về niềm vui và giao tiếp.', soMayMan: '2, 5, 6, 7, 8' },
    8: { ten: 'Cung Cấn', menh: 'Thổ', huongTot: ['Tây Nam', 'Tây Bắc', 'Tây', 'Đông Bắc'], huongXau: ['Đông Nam', 'Bắc', 'Đông', 'Nam'], mauSac: ['Vàng', 'Nâu đất', 'Đỏ'], vatLieu: ['Đá', 'Gạch thô', 'Đất nung'], moTa: 'Cung Cấn thuộc Thổ, chủ về sự ổn định và tích lũy.', soMayMan: '2, 5, 8, 9' },
    9: { ten: 'Cung Ly', menh: 'Hỏa', huongTot: ['Đông', 'Đông Nam', 'Bắc', 'Nam'], huongXau: ['Tây Bắc', 'Tây', 'Tây Nam', 'Đông Bắc'], mauSac: ['Đỏ', 'Cam', 'Xanh lá'], vatLieu: ['Gỗ sơn', 'Vật liệu nhân tạo'], moTa: 'Cung Ly thuộc Hỏa, chủ về danh tiếng và trí tuệ.', soMayMan: '3, 4, 9' },
};

const NGU_HANH_TUONG_SINH: Record<string, string> = { 'Kim': 'Thổ', 'Thủy': 'Kim', 'Mộc': 'Thủy', 'Hỏa': 'Mộc', 'Thổ': 'Hỏa' };
const NGU_HANH_TUONG_KHAC: Record<string, string> = { 'Kim': 'Hỏa', 'Mộc': 'Kim', 'Thổ': 'Mộc', 'Thủy': 'Thổ', 'Hỏa': 'Thủy' };
...
export class FengshuiService {
    ...
    private tinhCungSo(lunarYear: number, gender: Gender): number {
        ...
        let cung = gender === Gender.MALE ? (10 - base) % 9 : (base + 5) % 9;
    ...
    private tinhConGiap(year: number): { chi: string; ten: string; emoji: string; tinhCach: string } {
        const chi = CHI[year % 12];
        return { chi, ...(CON_GIAP[chi] ?? { ten: 'Không rõ', emoji: '❓', tinhCach: '' }) };
    }

    // ĐÃ FIX: Kiểu trả về khai báo chính xác { n, y, v } và đảm bảo đủ dấu ngoặc {}
    private tinhNapAmVaVan(canChiStr: string): { n: string; y: string; v: string } {
    ...
    private tinhTuHanhXung(chi: string): string {
        const groups = [
            ['Dần', 'Thân', 'Tỵ', 'Hợi'], ['Tý', 'Ngọ', 'Mão', 'Dậu'],
            ['Thìn', 'Tuất', 'Sửu', 'Mùi']
        ];
        const match = groups.find(g => g.includes(chi));
        return match ? match.join(' - ') : '';
    ...
    private tinhMenh(lunarYear: number): string {
        ...
        switch (menhValue) {
            case 1: return 'Kim'; case 2: return 'Thủy';
            case 3: return 'Hỏa'; case 4: return 'Thổ';
            case 5: return 'Mộc'; default: return 'Không rõ';
        }
    }
    ...
    private async isVipUser(userId: number): Promise<boolean> {
        ...
        try {
            ...
            const sub = await this.prisma.vipSubscription.findFirst({
                where: { userId, status: 1, startDate: { lte: now }, endDate: { gte: now } }
            });
            return !!sub;
        } catch { return false; }
    }
    ...
    async analyze(dto: FengshuiAnalyzeDto, user?: any) {
        try {
            ...
            let lunarYear = dto.calendarType === CalendarType.SOLAR ? this.lunarYearFromSolar(day, month, year) : year;
            ...
            const locationWhere = dto.location ? {
                OR: [{ city: { contains: dto.location } }, { district: { contains: dto.location } }]
            } : {};

            const takeLimit = isVip ? 8 : 4;
            const [houses, lands] = await Promise.all([
                this.prisma.house.findMany({ where: { direction: { in: huongCanTim }, ...locationWhere }, include: { images: true, category: true }, take: takeLimit, orderBy: { createdAt: 'desc' } }),
                this.prisma.land.findMany({ where: { direction: { in: huongCanTim }, ...locationWhere }, include: { images: true, category: true }, take: takeLimit, orderBy: { createdAt: 'desc' } }),
            ]);

            const freeData = {
                isVip,
                thongTinCaNhan: {
                    ten: dto.name, ngaySinh: dto.birthDate, loaiLich: dto.calendarType === CalendarType.SOLAR ? 'Dương lịch' : 'Âm lịch',
                    gioiTinh: dto.gender === Gender.MALE ? 'Nam' : 'Nữ', namAmLich: lunarYear, canChi,
                    conGiap: { chi: conGiap.chi, ten: conGiap.ten, emoji: conGiap.emoji, tinhCach: conGiap.tinhCach },
                    batMiBanMenh: {
                        napAm: napAmInfo.n, yNghiaNapAm: napAmInfo.y, vanMenh: napAmInfo.v,
                        tamHop, tuHanhXung, soMayMan: cungInfo.soMayMan
                    }
                },
                menhCung: { menh, cungSo, tenCung: cungInfo.ten, moTa: cungInfo.moTa, nguHanhSinh, nguHanhKhac },
                batTrach: { cat: huongCat, hung: huongHung },
                batDongSan: { nhaO: houses, datDai: lands, tongNha: houses.length, tongDat: lands.length }
            };

            const vipData = isVip ? {
                phongThuyChiTiet: { mauSacHop: cungInfo.mauSac, vatLieuHop: cungInfo.vatLieu },
                luuY: [
